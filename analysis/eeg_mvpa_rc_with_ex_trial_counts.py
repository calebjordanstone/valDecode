...
#### Note that htis version of the script drops extra trails from the reward phase so that there are equal trial numbers as the etinction phase #####
...
for path in epoch_paths:
    ...
    #### equalise rc trial numbers with ex trial numbers
    no_epochs_ex = len(epochs_ex['hi/aw/le/cue'])
    no_epochs_rc = len(epochs_rc['hi/aw/le/cue'])
    epochs_rc.selection = np.arange(len(epochs_rc))
    # drop epochs randomly
    # selections = [
    #     np.random.choice(epochs_rc[epoch_type].selection, size=no_epochs_ex, replace=True, p=None) 
    #     for epoch_type in epochs_rc.event_id.keys()
    # ]
    # drop epochs selectively from beginning
    selections = [
        epochs_rc[epoch_type].selection[no_epochs_rc-no_epochs_ex:] 
        for epoch_type in epochs_rc.event_id.keys()
    ]
    
    new_epochs_array = np.sort(np.concatenate(selections))
    epochs_rc = epochs_rc[new_epochs_array]
    beh_rc = beh_rc[new_epochs_array]
    ...
    phase = "rc" #NOTE: set this to be either "rc" (reward contingency) or "ex" (extinction phase); and comment out relevant line below
    ...
    SAVEPATH = "C:\\Users\\cstone\\OneDrive - UNSW\\Documents\\Projects\\my_experiments\\val_decode\\output_allTrials_8xcv\\cleaned_beh_files\\"
    beh_rc.write_csv(SAVEPATH + f'beh_rc_{subID}_with_ex_trial_counts_dropped_from_start.csv')
    ...
    # save csv 
    SAVEPATH = "C:\\Users\\cstone\\OneDrive - UNSW\\Documents\\Projects\\my_experiments\\val_decode\\output_allTrials_8xcv\\multiclass_decoding_results_files\\"
    #df_decfun.group_by(["event_type", "event_group", "y", "tpoint", "subID"]).mean() # average over 8 iterations of CV
    df_decfun.write_csv(SAVEPATH + f'dfun_{extension}_{phase}_{subID}_with_ex_trial_counts_dropped_from_start.csv')
